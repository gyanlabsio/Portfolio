...
exports.deleteProject = async (req, res, next) => {
    try {
        ...
        if (project) await project.softDelete(req.admin ? req.admin._id : null, "Admin soft delete");
        if (!project) {
            return res.status(404).json({ success: false, message: 'Project not found' });
        }
...
exports.toggleLike = async (req, res, next) => {
    try {
        ...
        const hasLiked = project.likes.includes(visitorId);
        
        if (hasLiked) {
            // Unlike
            project.likes = project.likes.filter(id => id !== visitorId);
        } else {
            // Like
            project.likes.push(visitorId);
        }
