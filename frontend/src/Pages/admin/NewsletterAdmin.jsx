    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {loading && !list.length ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {list.map(template => (
            <div key={template._id} className="bg-white border rounded-xl shadow-sm overflow-hidden flex flex-col">
              <div className="p-5 flex-1">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-lg text-gray-900">{template.name}</h3>
                  <span className={`px-2 py-1 text-xs rounded-full ${template.status === 'sent' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
              ...
              <div className="bg-gray-50 p-4 border-t flex justify-end gap-3">
                {template.status !== 'sent' && (
                  <>
                    <button onClick={() => handleEdit(template)} className="p-2 text-gray-600 hover:text-blue-600 transition bg-white border rounded shadow-sm">
                      <Edit size={18} />
                    </button>
                    <button onClick={() => handleSend(template._id)} className="p-2 text-white hover:bg-green-700 transition bg-green-600 rounded shadow-sm flex items-center">
                      <Send size={18} className="mr-2" /> Send Blast
                    </button>
                  </>
                )}
                <button onClick={() => handleDelete(template._id)} className="p-2 text-red-600 hover:bg-red-50 transition bg-white border rounded shadow-sm">
                  <Trash2 size={18} />
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
            ...
            <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
              ...
              <div className="flex justify-end pt-4 border-t">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2 border rounded-lg mr-4 hover:bg-gray-50">Cancel</button>
                <button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-lg">Save Template</button>
              </div>
            </form>
          </div>
        </div>
